...
foreach ($ms in $milestones) {
    ...
    $created = $null
    try {
        $response = $payload | gh api "repos/$Repo/milestones" --method POST --input - 2>$null
    ...
    if ($created) {
        ...
        $existing = @($allMilestones | Where-Object { $_.title -eq $ms.title })[0]
        if ($existing) {
...
foreach ($sprint in $backlog) {
    $msTitle = $sprint.milestone
    Write-Host "`n  >> $msTitle" -ForegroundColor Yellow
